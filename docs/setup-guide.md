...
**このファイルは外部サービスの初期設定手順だけを持つ。**仕組みと運用は次が正で、
ここに写しを置かない(2026-05 時点の workflow 一覧を載せたまま、全部が消えた後も
残っていた。2026-09-26 に削除)。

- 構成・定期実行の一覧: [architecture-2026-07.md](architecture-2026-07.md)
- 運用手順・判断の記録: [task-playbook.md](task-playbook.md)
- データ項目: [event-data-prompt.md](event-data-prompt.md) / [api.md](api.md)
...
定期の確認は日次メール(健全性チェック・整合監査・要判断キュー)と
週次メール(巡回・補完レポート)に集約している。GitHub Issues は使わない。
手順は [task-playbook.md](task-playbook.md)。
