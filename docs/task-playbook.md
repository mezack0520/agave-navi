...
- **`script.google.com/home/triggers` を素で開くと仕事用アカウントが出る。**
  Chromeには複数のGoogleアカウントがログインしており、既定(`/u/0`)は
  日によって別のアカウントを指す。仕事用が出た場合は Gantt chart の
  無関係なトリガーが「多数のうち25個」だけ並び、
  ...
  **`/u/N` の番号はここに書かない。**並び順が入れ替わるので、書いた瞬間に
  古くなる(2026-08-30 は `/u/1/` が個人、2026-09-06 と 09-09 は `/u/0/` が個人)。
  開いた画面のアカウントを毎回読んでから判定する。手順は下の
  「Googleアカウントの `/u/N` は入れ替わる」を見る。
  判定の材料は、ページに `agave-navi フォーム→GitHub連携` の行が
  出ていることだけ。
