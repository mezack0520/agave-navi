...
7. **台帳に `taskId` を足したら、同じ回にそのタスクの SKILL.md へも
   ...
   **「記録していない」と「動いていない」を先に切り分ける（2026-09-15）。**
   `task_run_never_recorded` が出たら、まず
   `task-reports/<taskId>_<日付>.md` が**あるか**を見る。

   - レポートがある → 記録の問題。SKILL.md に `record-run.py` の行を入れる。
   - **レポートも無い → 成果物を `git log` で見る。それも無ければタスクが
     動いていない。** Coworkデスクトップのスケジュールタスク一覧で、
     有効か・最後の実行がどうなったかを見る。

   **※ この2分岐は 2026-09-16 に3分岐へ訂正した（§3「レポートが無くても、
   成果物を見れば動いたかが分かる」）。レポートが無くても commit は在る、
   という回が現にある。ここだけを読んで「動いていない」と結論しないこと。**

   2026-09-15、この検査を前者と決めつけて SKILL.md を2回直そうとし、
   目崎の時間を2往復ぶん使った。
   **台帳が空なのは症状で、原因ではない。**成果の指標が悪化していないかを
   必ず併せて見る。

   **ただしその日の「実際は後者だった（＝動いていない）」は誤り（2026-09-16 訂正）。**
   `agave-navi-eyecatch` は 09-15 に `13e5313`「アイキャッチ4件追加」を押している。
   残数が 41 → 54 に増えたのを止まっている証拠と読んだが、同じ期間に
   `upcoming` も 144 → 163 に増えており、**残数は母数が増えれば増える。**
   差の向きだけでは働きを測れない。詳細は §3 の同題の項。
...
- **掲載申請フォームはもうポーリングしない。GAS連携が2026-08-24に稼働した。**
  **※ ただしシートを開く手順自体は毎回やる（2026-09-07 追記）。**下の「gviz CSV での
  ...
  「平常時は使わない」を字義どおり読んでシートを開かないと、
  送信の無い日は `sheetRows` を誰も測らず `sheet_measurement_stale` が鳴る。
  **実機GASは 2026-09-08 版で `countSheetRows` を持つ（2026-09-18 訂正）。**
  持たないと書いていたのは 09-06 時点の事実で、09-07 の貼り直しで解消している。
  それでもタスク側が測るのは、**GASが書くのはフォーム送信の瞬間だけ**で、
  送信が無ければ実測日が動かないから。ポーリングをやめたのは
...
- **GASのトリガーは「登録されている」だけで、実フォーム送信で発火した実績がまだ無い。**
  **※ この結論は 2026-09-06 に訂正済み（§3 の「フォーム→GAS→push→メールは、端から端まで通った実績がある」）。08-31 / 09-11 の送信で現に発火している。ここだけを読んで「未証明」と書かないこと（2026-09-15 追記）。**
...
- **組み込みブラウザは `document.hidden === true` のまま動く。
  ...
  `scroll` イベントが出ず、IO のコールバックは**初回すら**呼ばれない。
  そのためスクロールで出し入れするUIは、実物が正常でも「出ない」と見える。
  実測: `scrollTo` を10段階、各600ms待って `scroll` 0回 / IO 0回。
  `.aff-bar` は `update()` がスクロールで呼ばれないため `is-shown` が付かず、
  **上の「50%までスクロールして見る」だけでは壊れていると誤読する。**
  手で発火させること:

  ```js
  window.scrollTo(0, total * 0.6);
  window.dispatchEvent(new Event('scroll'));   // ペインが描画しないので手で
  ```
  ...
  **同じ理由で CSS トランジションも進まない。** `transform .22s` を持つ要素は
  `getComputedStyle` が開始値を返したまま固まり、クラスを足しても値が変わらない。
  位置を測る前に `*{transition:none!important}` を注入する。
  これを知らずに「クラスは付いているのに transform が動かない」を追うと、
  セレクタの特異度やCSSの版ずれを疑って半日溶ける(2026-09-14 に実際に溶かした)。

  **判定の順序:** 出ない → まず `document.hidden` と
...
- **トリガー画面の「前回の実行」を、シート最新行のタイムスタンプと突き合わせる（2026-09-14）。**
  `/home/triggers` は毎回開く手順に入っているのに、読んでいるのは
  **行が在るか**と**エラー率**だけだった。だが同じ行に「前回の実行」が出ている。
  これが最新行のタイムスタンプと秒単位で一致すれば、
  **最後の送信で `onFormSubmit` が現に発火した**ことになる。
  今日の実測: 前回の実行 `2026/09/11 20:24:31` / 最新行 `2026/09/11 20:24:30`。
  `sheetRows` と `processed` の突き合わせは、**取りこぼしが1日ぶん溜まってから**しか鳴らず、
  しかも「GASが発火しなかった」のか「タスクが処理していない」のかを分けない。
  こちらは既に開いている画面で、送信のあった最新の1件について直接答えが出る。
  ずれていたら（前回の実行が最新行より古い）GASが発火していないので `backfillFromSheet`。
  送信ゼロの期間は当然古いままなので、**比べる相手は「今日」ではなく「シート最新行」**。
  2026-09-15 も一致を確認: 前回の実行 `2026/09/11 20:24:31` / 最新行 `2026/09/11 20:24:30`。
...
ならない」と言っているが、**出店者の告知を読む価値が無いとは言っていない。**

ナゴリバ(2026-09-20 広島)は 09-10 から4日間「主催が特定できない」で止まっていた。
アグリゲータ4社と出店者IG3件を突き合わせても主催に届かず、09-13 に
`unverified` で見送っている。今日、出店者の投稿本文を**最後まで読んだ**ら
「詳しくは主催者、和さんのインスタをご参照ください @nagomitaniku」と書いてあった。
同じ文言が3人の出店者に出ていた。開いたら会期・時間・整理券の運用・
出店15組の一覧まで揃った一次情報があり、その場で掲載できた。

**探していたのは主催の告知で、目の前にあったのは主催の名前だった。**
アグリゲータの要約も検索結果のスニペットも「誰が主催か」を落とす。
出店者は落とさない。自分の集客のために主催へ誘導するからで、
これは出店者の告知にしか無い情報。

手順: 出店者の告知しか無い回は、見送りを決める前に
**出店者の投稿本文から `@` で始まるハンドルを全部拾い、
「主催」「主催者」の語の近くにあるものを1回開く。**
検索結果のスニペットは本文の途中で切れるので、ここは投稿を開く必要がある。

### 主催を1件開くと、その主催の翌日の別イベントが一緒に出る (2026-09-14)

ナゴリバの主催 @nagomitaniku を開いたら、2日前の投稿に
「9/21 農マル園芸吉備路農園（岡山）多肉植物『The販売会』」があった。
**前日ナゴリバ（広島）に出た生産者が、翌日そのまま岡山で売る**という形で、
LEAFLA・NextMeet・PUKUBOOK のどれにも出ていない。

既に §3 に「IGプロフィールの `img.alt` は同じ主催の別イベントも見せる」があるが、
あれは**同じシリーズの別の回**の話だった。こちらは**別名・別会場・別県の単発**で、
名前でも会場でも引けない。遠征のついでに開く回は、そもそも告知が主催の
タイムラインにしか無い。

**遠征型の主催を開いたら、前後1週間ぶんの投稿を見る。**
連続する日付の別会場が並んでいたら、それは全部別イベント。

### 出展者1組の取り止めが、イベントの中止として鳴る (2026-09-14)

`cancel_suspects` が `ore-no-plants-collection-2026-09` を
「中止・延期の語: 中止となりました」で出した。本文は

> 9月19日（土）、20日（日）に出展予定のkuwa.botanicalさんは
> 都合により出展中止となりました。

で、降りたのは**出展者1組**。イベントは会期のまま開催される。

これは 2026-09-09 に整理した誤検知3型（今日の日付・天候の条件文・
プラグインの既定表示）のどれでもない。**条件文ではなく言い切っている**ので
`drop_conditional()` を素通りし、`CANCEL_WORDS` の「中止となりました」に当たる。
会場が出展者の入れ替わりを本文に書き続ける頁では、これが毎日鳴り続ける。

対処: `check-cancelled.py` に `VENDOR_CANCEL` を足し、
**主語が出展・出店・出品・参加であるときだけ**落とすようにした。
「開催中止」「イベント中止」は主語が違うので残る。
自己テストに `FIX_VENDOR_CANCEL` を追加（修正前の本文に当てると
`中止となりました` が出ること、修正後に出ないこと、
「台風のため開催中止のお知らせ」は引き続き出ることを確認済み）。

**誤検知の型を数えるときは「言い切りかどうか」ではなく「主語は誰か」で割る。**
2026-09-09 の整理は「言い切っているものは拾う」を軸に置いていたが、
言い切っていても主語がイベントでなければ関係がない。

### PUKUBOOK の「注目イベント」は常設で、毎回候補に出る (2026-09-14)

`pukubook.jp/events/` の FEATURED EVENTS 枠に GREEN×EXPO 2027（会期6か月・
2027年3月〜9月）が入っている。巡回のたびに未掲載として出てくるが、
主題は園芸・造園の総合博で `outOfScopeGenres` に当たる。

**長期・常設の掲載は、1回の見送りを `rejected-events.json` に書くだけでは足りない。**
会期が長いので `rejected_revisit_expired` の期限にも当たらず、
...
### 指標の「曜日差」は、途中の段差と見分けが付かない (2026-09-14)

`audit.metric_weekly_cycle_unmodeled` は平日と土日の**中央値をそのまま**比べていた。
標本の途中で水準が一段上がる指標では、平日と土日が時間軸上の別の位置に散らばるため、
中央値の差が「週の周期」ではなく「途中の段差」を測る。

2026-09-14 に `events_with_image` で鳴った。09-08 の画像一括取り込みで 67→142 に上がり、
平日中央値 67 / 土日中央値 149 と出た。値は単調増加で曜日とは無関係で、**誤検知だった。**
この検査自体が「誤検知を減らすため」に入れたものなので、放置すると目的と逆に働く。

直し方は**各土日を、その前後3日の平日の中央値と比べる**(局所差)。段差は前後で
打ち消し合って消え、曜日による上下だけが残る。実データで局所差 +6/基準143(4%)となり
止まった。合成データ(平日2・土日10)では +8/基準2(400%)で従来どおり鳴る。
**「比較の両群が時間軸上で偏っていないか」は、群別の中央値を取るすべての検査に効く。**

### 商品枠の説明文は季節の語を持つが、出す月は season が決める (2026-09-14)

`amazon-links.json` の品目は `season`(旬の月の配列)で順位が上がるが、
利用者が読む `label` は季節の語を直に書いている。両者がずれると
**その季節が終わってからも同じ文が本番に出続ける。**
2026-09-14 に詳細ページの最上位が「梅雨と夏の蒸れ対策に効く送風」だった。9月に梅雨は無い。

`audit.affiliate_label_season_mismatch` が label の季節語と `season` の月を突き合わせる。
出たら label から季節の語を外して通年で読める文にするか、`season` をその語の月に狭める。
`season` を持たない品目は通年扱いなので対象外(ガイドの本文が文脈を与える)。

**この型は「表示されている文が正しいか」であって、「要素が在るか」では落ちない。**
`.rk-card` が3枚出ていることは何度も確認していたが、中身の文は誰も読んでいなかった。

### 表示側だけが住所の括弧書きを落としていた (2026-09-14)

`events.json` の `location` は「会場名（都道府県…住所）」の形で
会場名と住所を1つの文字列に入れている。
表示側は `sitelib.venue_display` と `build-detail-pages.split_venue` で
括弧書きを落としているが、**JSON-LD だけが生の `location` を
`Place.name` にそのまま入れていた。**
結果、リッチリザルトの会場欄が「宮島口しゃもじ広場（広島県廿日市市宮島口1丁目）」と
括弧ごと出る回が17件あり、`PostalAddress` 側は `addressRegion` に県名だけで
`streetAddress` は常に空だった。
`location` が丁目・番地だけの回(「愛知県春日井市坂下町4-224-7」)は、
施設名の無い会場を名乗ることになる。

`make_event_jsonld` が `_split_place` を通して、住所は `streetAddress` へ、
番地だけの回は `Place.name` ごと省くようにした。
`audit.jsonld_place_name_not_venue` が生成済みHTMLの `Place.name` を読んで検査する。

**同じ値を読む場所が3つあり、直っていたのは2つだった。**
`place_field_not_a_venue` は `location` を case (a) の対象から外している。
「会場名が location にあるなら venue を消して location へフォールバックさせる」が
その検査の対処法なので、フォールバック先の品質は誰も見ていなかった。
**逃がし先を持つ検査は、逃がし先が正しいかを別に見る必要がある。**

括弧の中が都道府県名で始まるものだけ住所とみなす規則は `split_venue` と揃えてある。
「コロレ（横）」「グランメッセ熊本（全天候型完全屋内）」のような但し書きは落とさない。
市名だけの括弧書き(「シマムラ園芸 第2ハウス（越谷市）」)は36件あるが、
但し書きと見分けが付かないので触っていない。

地図の埋め込みクエリも同じ生の `location` + 県名で、県名が二重になる回が14件ある。
**こちらは直していない。**Google Maps が今どう解決しているかを測れないので、
直した結果が良くなったと言えない。測れない直しは入れない。

### 絞り込みは「その情報源にその情報が在るか」で決める (2026-09-15)

`coverage-sweep.py` の NextMeet の段は、2026-09-10 に機械化してから
**実質ゼロ件しか判定していなかった。**`fetched: 4` / `errors: []` /
`gaps: 0` を返し、`coverage_source_missing` も満たすので、
`coverage-gaps.json` は最も健全な見た目のままだった。

穴は3つ重なっていた。**どれも単独では「動いている」と読める。**

1. **`in_scope` を当てる相手を間違えていた。**
   IN_SCOPE の語を要求する絞り込みは、LEAFLA のように
   「<会場>で<名前>を開催、アガベや塊根植物を展開」という**文**が来る
   情報源にしか効かない。NextMeet の月別は `<h3>` に**イベント名そのもの**が
   来るので、名前にジャンル語を持たない回が全部落ちる
   (「緑楽宴」「植祭」「BOTANICBOMB vol.11」「ぶらりぷらんつ」)。
   取得86件のうち通っていたのは16件(19%)。
   NextMeet は `/plants/` 配下の**植物イベント専用インデックス**なので、
   そもそも対象性は情報源の側で保証されている。`prescoped=True` で
   IN_SCOPE を外し、OUT_OF_SCOPE だけを当てる。
   **前段を外すと、後段が唯一の関門になる。** OUT_OF_SCOPE は
   IN_SCOPE が前にあるうちは手薄でよかった(「洋ラン展」は対象語を
   持たないので自然に落ちていた)。listing-policy の `outOfScopeGenres` に
   合わせて洋ラン・メダカ・山野草・造園・有用植物を足した。

2. **`matches` に同値の検査が1つも無かった。**
   関数全体が「長い文の中に短い名前が出るか」を見る向きにできており、
   包含の下限は6文字、特徴語の下限は5文字。名前どうしを比べると
   **「ナゴリバ」対「ナゴリバ」が一致しない**(4文字でどちらにも届かない)。
   1 を直した直後、掲載済みの回が10件中5件、取りこぼしとして出た。
   同じ向きの偏りで
   - こちらが会場名を括弧で足した回(「多肉植物『The販売会』（農マル園芸吉備路農園）」)
   - 向こうが短い通称で書く回(「KKT vol.6」対「KKT多肉FES vol.6」)
   も外れる。前者は**掲載した翌日から毎日**候補に出ていた。

3. **月の窓が1か月飛んでいた。**
   補完の `while` が `(y, m)` を進めてから append しており、
   窓を抜けた直後の月を必ず飛ばす。窓が10/30までの日に
   `['2026-09','2026-10','2026-12','2027-01']` を取りに行き、
   **11月を一度も見ていなかった。**3→4か月に広げた狙いがその月だった。

実測 61件対象/1件候補 → 145件対象/7件候補。うち5件が本物の取りこぼしで、
その日のうちに4件掲載した(第6回蒲郡多肉フェスタ・JSS北関東支部・
Our Plants ver.2.04・enjoy place 番外編)。

**教訓は3つとも同じ形をしている。**
ある情報源のために書いた判定を、別の形をした情報源にそのまま当てた。
LEAFLA 向けの `in_scope`、LEAFLA 向けの `matches`、
日付で決まる窓に月別の補完。
**情報源を足すときは、取得の段だけでなく判定の段も自分の形に合わせる。**
そして機械化した直後に「何件が判定まで届いたか」を数える。
`fetched` は取得の数であって、判定の数ではない。
`stats['in_scope_<source>']` を情報源ごとに出すようにした。

### 閾値を上げる前に、別の軸で外せないかを見る (2026-09-15)

`audit.rejected_but_listed` が、見送り
「GOLDEN TIME vol.8 (… GREEN FEEL・福島県福島市)」を掲載
「TRILLGREEN SPECIAL EVENT SILVER WEEKEND」(山形県米沢市)と
**`green` の5文字だけで**一致させた。名称4文字＋会場4文字の同時一致で
鳴る規則で、この分野に頻出する語がちょうど両方に入った。

コメントには既に「一般語(植物 / プランツ / plants / popup)が引っかかる」と
書いてあり、対処は閾値だった。だが閾値は上げれば本物を落とす。

**同じ日に別の県でやっている回は同じ回ではない。**
見送り側の `name` は「(開催日 会場・都道府県…)」の形で書く約束なので、
62件中48件が県名を持っている。開催地が分かっていて違えば外す。
語の長さで測るのをやめて、別の軸を1つ足すほうが副作用が無い。

### 自分で作った期待値が、実測した安全性と食い違ったら (2026-09-15)

`matches` の自己テストに「『植祭』対『第3回 植祭り』は一致しない」を
置いた。**`植祭り` という回は実在しない。**思い付きで書いた仮の期待値だった。

そのあと特徴語の部分集合で照合する経路を足したら、この仮の検査だけが落ちた。
危ないのは、ここで**検査に合わせて実装を歪めたくなる**ことで、
実装の側は events.json の同日ペア全件で誤一致0組を確認してある。
測ったものと思い付きが食い違ったら、落とすのは思い付きのほう。
**実在しない入力に対する期待値は書かない。**書くなら、その形を
実際に見た日と出典を添える。

### 同じ事実を2つの検査に出すと、片方が片方を埋める (2026-09-15)

`task_run_never_recorded` は 2026-09-12 に「`task_run_gap` に毎日出続けて
本物の抜けが埋もれる」という理由で新設した。**だが `task_run_gap` の側から
その回を取り除いていなかった。**結果、`agave-navi-eyecatch` は
09-10〜09-14 の5日を `task_run_gap` に積み上げたまま、
`task_run_never_recorded` にも1行出ていた。分けた意味が無い。

§6 には「台帳の抜けを数える前に `task_run_never_recorded` を先に引いてから
『全タスク同日か』を見ること」と書いてある。**人手の手順として書いた引き算は、
機械の側に移せる。**移すまでは、週次点検のたびに人が同じ引き算をやり直し、
やり忘れた回だけ判断を誤る。

`task_run_gap` は `since` 以降の記録が1件も無いタスクを飛ばすようにした。
実測 3件→2件。残った2件は `agave-event-update` と `event-monitor` の
09-11 のみで、**全タスク同日＝環境側**が1行で読める形になった。
判定は `_never_tids` の1か所だけが持ち、文面を作る側は条件を持たない。

**検査を新設して「別の問題として分けた」と言うときは、元の検査から
その回を引いたかを確かめる。**分けただけでは分かれていない。

### 「変わった」は、増えたのか減ったのかを持たない (2026-09-15)

`cancel_suspects` の「公式ページが変わった(本文)」は、本文の sha256 が
前回と違うことしか言っていない。**告知が足された回と、告知が消えた回が
同じ1行になる。**前者は毎日出る雑音、後者は必ず人が見るべきもの。

食虫植物祭2026 が鳴った回を開くと、本文にはバス便と懇親会のチケット
販売リンクが**増えて**いて、中止の語は無かった。そこで「追記だから雑音」と
読んで閉じかけたが、`signature.textLen` を世代で追うと
**3041→1334字（-57%）で減っていた。**見えていたものと逆だった
（この頁は Notion 系のJS描画で、取得のたびに返る量が変わる）。

- `check-cancelled.py` は `prevTextLen` を持ち回り、`why` に字数の増減を書く
- `audit.cancel_watch_body_shrunk`(info) が4割以上の減少だけを別に出す
- 判断を `scripts/cancel-reviewed.json` に書けば黙るのは `cancel_suspects` と同じ

実測（`cancel-watch.json` の27世代）: 4割以上の減少はこの1件だけ。
増えるほう（greensnap / kourep / ore-no-plants）は毎週動いている。
**同じ材料でも、向きを見るだけで雑音と本物が分かれる。**
ハッシュで「同じかどうか」を測る検査を足すときは、
同じ場所に**量**も残しておくこと。後から向きが要る。

### shallow の境界に当たったコミットは、ツリー全体を「変更」として返す (2026-09-16)

`ci_generated_paths_drift` が CI では 126件の urgent を出しているのに、
**完全な履歴を持つ手元で同じ監査を回すと 0件**だった。

原因は `git show`。CI の checkout は `fetch-depth: 10` で、
検査は `git log -5` で CI コミットをたどる。CIコミットが立て込んだ日は
後ろのほうが shallow の境界に来る。**境界のコミットは親を持たないので、
`git show --name-only` が差分ではなくツリー全体を並べる。**
その結果 `.gitignore` `CNAME` `404.html` `.github/workflows/*` のような
手書きファイルがまとめて「未分類」として出る。

この検査には既に「CIコミットが1件も見えないときは鳴らさない」逃げ道がある。
だが**見えるが親が無い**という中間の状態を見ていなかった。
`git rev-parse -q --verify <sha>^^{commit}` で親の有無を見て、
無ければその回は判定しない（件数は note に出す）。

再現と検証: `git clone --depth 11` で境界を作ると、修正前 126件・修正後 0件。
境界の位置は HEAD からの距離で決まるので、**深さを1つ変えるだけで
出たり出なかったりする。**「今日は出たが昨日は出なかった」を
データの変化と読んではいけない。

**環境に依存する検査は、同じデータを別の深さの clone で2回測る。**
手元で0なのに CI で鳴っているなら、疑うのはデータではなく取得のしかた。

### 生産者・専門店の「巡業」は、会場ごとに別イベントとして落ちる (2026-09-16)

Jolie Nursery（レアアロイドの生産者）は全国のホームセンターを回って即売会を開く。
当サイトに載っていたのは 8月のプロトリーフ二子玉川の1回だけで、
**9/5-6 須坂 / 9/12-13 札幌白石 / 9/19-20 茨木 / 9/26 瑞穂 / 10/10-11 新潟 の
5回は1つも載っていなかった。**前2つはもう終わっている。

会場が毎回違うので、会場でも名前でも引けない。
アグリゲータも「◯◯が△△店で即売会」という出店者視点の見出しにするので、
`vendorBoothInHostEvent` と見分けが付きにくい
（実際はホームセンターの区画を1社が単独で使う回なので `notApplies` のほう）。

**効いたのは、その生産者の公式サイトのイベント一覧を1回開いたこと。**
`jolienursery.co.jp/pages/events` は開催予定と実績を1ページに並べており、
住所・時刻・共催まで揃った一次情報が5回ぶん一度に取れた。
§3 の「裏取りで会場公式を開いたら一覧まで1階層上がる」「主催IGを開いたら
前後1週間を見る」と同じ効き方で、**こちらは出店者側の公式サイト**。

手順: 同じ生産者・専門店の名前が2回以上候補に出たら、
その会社の公式サイトに「イベント情報」「出店予定」のページが無いかを見る。
あれば1回の巡回でシリーズ全部が取れる。

### 会場公式の記事は、投稿日が開始日として入り込む (2026-09-16 / 2026-09-12 の再発)

`rare-foliage-sale-protoleaf-futako-2026` が
`date=2026-08-15`〜`dateEnd=2026-08-30` の16日間になっていた。
プロトリーフの告知は題も本文も **8/29(土)・8/30(日)** の2日間で、
`2026年8月15日` は記事の**投稿日**である。

09-12 の `event_start_is_page_meta` と同じ形だが、**その検査では拾えない**。
判定材料を書く `check-cancelled.py` は開催60日以内の回しか巡回しないので、
**終わってしまった回は検査の母集団の外**にある。
つまりこの型は「気づかないまま past に落ちると永久に残る」。

見つけたのは別件（Jolie Nursery の巡業を調べていて公式の一覧と突き合わせた）で、
repo 側から見つける経路は無い。掲載時に間違えるとほぼ取り返せないので、
**会場公式・まとめ頁から日付を採るときは【開催日時】の行だけを読む**を
掲載のその場で守るしかない。直すときは `autoDateUpdate: false` を付ける
（終了済みの回に自動更新の利得は無く、同じ頁からまた投稿日を拾われる）。

### 絞り込みの期待値を、取りこぼしたままの挙動で固定しない (2026-09-16)

`coverage-sweep.py` の LEAFLA の段は、見出しに `IN_SCOPE` のジャンル語
（アガベ・多肉・塊根…）が1語も無い回を落としていた。
2026-09-15 に NextMeet で直したのと**同じ穴が LEAFLA 側に残っていた**。
あちらは `prescoped=True` で前段ごと外して済んだが、
LEAFLA は植物全般を載せるので前段は外せない。

見つけ方は探索の副産物だった。PUKUBOOK の一覧に
「植物天国 in 白山市場」(9/20-21・新潟・観葉植物の展示即売会・植物ブース11組)が
出ていて、`coverage-gaps` は 0件・`errors` 0 のいちばん健全に見える状態だった。
開催4日前で、45日の窓のど真ん中にいる。
LEAFLA は 08-28 に記事を出しており**取得もできていた**。
落としていたのは取得ではなく判定で、見出しが
「植物天国『植と食』のマーケット…」とだけ名乗っていたから。

**いちばん効いたのは、自己テストの期待値を疑ったこと。**
`SELFTEST_EXPECT_SCOPE` は「ワイルドプランツ路地裏のギボウシが
**第1回ボタニカルX**2026を開催、珍しい植物と…」を `False`(範囲外)と
書いていた。この回は `botanical-x-vol1-2026` として**掲載済み**である。
つまり検査は、取りこぼしていた当時の挙動をそのまま正解として固定していた。
**期待値は「その時どう動いたか」ではなく「当サイトがその回を載せるか」で決める。**
ここが `False` のままだと、総称を足した瞬間に検査が落ちて、
「直したら検査が壊れた」と読んで元に戻すことになる。

直し方は3つ、いずれも実測してから入れた。

1. `IN_SCOPE` に総称（`植物` `ボタニカル` `botanical` `観葉` `plants` `plant`）を足す。
   `園芸` は入れない（資材セール・園芸講座を大量に連れてくる）
2. 流れ込む雑音は `OUT_OF_SCOPE` で落とす。**実測した雑音だけ**を足した
   （バラ展・シンポジウム・苔玉・夜間開園・絵画・彫刻・写真展・標本・
   生け花・フラワーアレンジ・寄せ植え・ラン科・菊花・朝顔・角鉢展・器展）。
   先回りして想像で足さない
3. 名前が共通語だけで出来ている回の照合。`tokens()` が空を返す掲載名が
   435件中16件あり（`BOTANICAL LIFE 2026` `PLANTS FES vol.4` など）、
   普段は `nn in n` が拾うが、**向こうが名前の途中に通称を挟むと外れる**
   （「On the **オンプラ**Plants Vol.09」）。共通語のまま
   **全部が・その順で**出ることを求める `generic_only_match` を足した

実測: `in_scope` 42→94件、取りこぼし候補 0→13件。
13件は雑音ゼロで、全部が判断の要る回だった。うち一次情報で裏取りして
掲載3件（植物天国・第肆回草力祭・American and Plants）、
見送り3件（夜アンビエン・KAKU PLANTS POT展・valo plants room）。
誤って「掲載済み」に倒していないかの検算は、`events.json` の**同日ペア全件**で
`generic_only_match` の誤一致が0組、その日の候補13件のうち
当たったのが On the Plants の1件だけ、で取った。

**`gaps: 0` は「取りこぼしが無い」ではなく「判定を通った候補の中に無い」。**
`fetched` が取得の数であって判定の数でないのと同じことが、
`gaps` と「実際の取りこぼし」の間にもある。
2026-09-15 に `stats['in_scope_<source>']` を出すようにしたのは
まさにこれを見るためで、`in_scope_leafla` は 42 だった。
**取得45日ぶんに対して対象42件は少ない。**その比は毎回出ているのに、
低いこと自体を誰も見ていなかった。

### レポートが無くても、成果物を見れば動いたかが分かる (2026-09-16)

§1 の 7 に 2026-09-15 で「`task_run_never_recorded` が出たら
`task-reports/<taskId>_<日付>.md` の有無を見る。**レポートも無ければ
タスクが動いていない**」と書いたが、**この二分は足りない。**

今日の実測: `agave-navi-eyecatch` はレポートが 2026-09-09 を最後に無く、
台帳も `since=2026-09-10` 以降ゼロ。前者の規則どおりなら「動いていない」になる。
ところが `git log` に **`13e5313`(2026-09-15)「アイキャッチ4件追加、目視で2件を見送り」**
がある。`images/events/` に4枚入り、`events.json` も更新されている。
**この commit を出せるのはあのタスクだけで、CI には作る経路が無い**
(`.github/workflows/` と `scripts/*.sh` に `eyecatch` を含むものは0件)。
つまり 09-15 は**動いて本番に出しているのに、レポートも台帳も書いていない**。

状態は3つある。

| 台帳 | レポート | 成果物 | 読み |
|---|---|---|---|
| 無 | 有 | 有 | 記録だけの問題。SKILL.md に `record-run.py` を入れる |
| 無 | **無** | **有** | **同じく記録だけの問題。レポートの行も一緒に入れる** |
| 無 | 無 | 無 | タスクが動いていない。スケジューラ側を見る |

**真ん中を見落とすと、直せる相手を「動いていない」に倒す。**
09-15 はそれをやりかけた（あの日は指標 `eyecatch_backlog` が
41→54 に増えているのを「止まっている証拠」と読んだが、
背景の `upcoming` も 144→163 に増えており、**残数は母数が増えれば増える**。
差の向きだけでは働きを測れない）。
...
それは転記の起点をどこに置くかの話で、**起動したかの判定には commit のほうが強い**。
レポートは書き忘れられるが、成果物は書き忘れたら成果が出ない。
...
### 漢字・仮名が混ざった短い固有名は、照合語から丸ごと消える (2026-09-17)

`coverage-sweep.py` の `tokens()` は文字クラスごとに語を切る
(`[㐀-鿿]{2,}` / `[぀-ヿ]{3,}` / `[A-Za-z0-9]{3,}`)。
**日本語のイベント名は3つが混ざるので、混在した短い固有名はどのクラスにも
届かず、照合語が1つも残らない。**

「植ノ宴」は 植(漢1)・ノ(カナ1)・宴(漢1) に割れて全部落ち、
`tokens('植ノ宴 -4- in 熊本')` は `['熊本']` だけだった。
そのため LEAFLA の「植ノ宴 in KUMAMOTO2026が熊本で開催、…」は
掲載済みの回に当たらず、毎日「取りこぼし」として出ていた。
`bare` 側(NextMeet)には数字を落として包含を見る逃げ道が
2026-09-15 に入っているが、**LEAFLA は名前ではなく文なので包含では拾えない。**
同じ理由で会場も外れる。「道の駅 仁保の郷 屋外 青空デッキ」は
`仁保`(2文字)しか当たらず、語数2にも長さ5にも届かなかった。

直し: `_TOKEN_MIXED_RE = [㐀-鿿぀-ヿ]{3,}` を足し、クラス別の語と
**集合で合わせる**(既存の語は減らない)。これで `植ノ宴` `道の駅` `仁保の郷` が
語として残る。

検算は repo のやり方どおり2つ。
(1) 自己テストに3件追加(当たること・同日の別会場を巻き込まないこと・会場側)
(2) `events.json` の**同日ペア全件**で誤一致を修正前後で数えた。
名前どうしは 6組 → 6組(**同一の集合。増えていない**)、
名前×別会場は 1組 → 0組(`Collect Plants Vol.3` と `グランメッセ熊本` の
組が消えた)。**増やさずに減った**ので入れた。

**語の切り出しを文字クラスで分けるときは、混ざった語が落ちないかを見る。**
落ちても例外は出ず、`gaps` に1行増えるだけで「取りこぼし候補」の顔をする。

### アグリゲータのトップ一覧は、先の回を出さない (2026-09-17)

PUKUBOOK の `/events/` は「今後のイベント」に近い日付ぶんしか並べない
(今日は 2026-09-21 まで)。**個別ページは先の回も持っている。**
9/26-27 の Onokoro Green Festa は `/events/onokoro-green-festa-2026` に在るのに
トップには出ておらず、前日 09-16 の巡回で見えていなかった。
`manual-sweeps.json` の `lastScope` は正直に「2026-09-21 まで表示分」と
書いてあり、**手順は守られていて、見えていた範囲が狭かっただけ。**

台帳に日付を書くだけでは「どこまで見たか」は次の回に伝わらない。
`lastScope` に**窓の右端**を書くこと(いま書いてある)。そして
**一覧の右端が近い日で切れている回は、それより先は検索で引く。**
今日は Google の `"<主催名>" 2026` で会場公式に届き、1回で確定した。

### watch-sources は「一度でも掲載した主催」しか見ない (2026-09-17)

`generate-watchlist.py` の docstring は「手動でウォッチリストを管理する
必要はない」と書いている。**それはループに入った後の話で、入口が無かった。**
watch-sources.json は events.json からの導出なので、
**一度も掲載したことがない主催は、何度イベントを開いても永久に
ウォッチ対象にならない。**

アガベ専門店モンスターズハウス(名古屋・2店舗)は 9/12〜15 に4日間の
NAMED AGAVE PARTY を2店舗同時で開いていた。当サイトは1件も載せていない。
知ったのは `coverage-gaps.json` の `truncatedUnresolved` からで、
**終わった後**だった。LEAFLA の記事は開催前日の 09-11。
店舗イベントの告知は数日前にしか出ないので、Step0 のキーワード検索が
その週に当たらなければ落ちる。そして掲載実績が無いから、次回も同じ形で落ちる。

対処: `watch-seeds.json` を作り、`generate-watchlist.py` の `load_seeds()` が
`igAccounts` へ `source="seed"` で混ぜる。掲載実績ができたら導出側が
同じ handle を持つので、seed は消してよい(既存は上書きしない)。
読む側は `audit.watch_seed_dropped`(urgent)。
**データに書いた値は、読む側を同じコミットで足す**(`blockedUrlDomains` /
`_reasonTypes` と同じ型)。

**取りこぼした主催は、その場で seed に足す。**「次から気をつける」は
記録ではないので、次の回に何も伝わらない。

### 2会場2日の回は、会場ごとに分ける (2026-09-17)

鹿鳥風月は 9/19 が FUKUOKA EFFECT(福岡市西区)、9/20 が ゆくはし植物園
(行橋市)で、主催の告知も「2Daysになります」と1件で書いている。
だが `venue` は1つしか持てず、地図・JSON-LD の `Place.name`・会場ランディングが
すべてそこを見る。2会場を1行に入れると3つが同時に壊れる。
2026-09-16 の Jolie Nursery の巡業と同じ形にして2件に分けた
(`kachofugetsu-fukuoka-effect-2026-09` / `kachofugetsu-yukuhashi-2026-09`)。
日付が違うので `duplicate_name_date` にも `duplicate_event_same_source` にも
当たらない(後者は同日+同出典が条件)。
**同じ出典URLを2件が共有するが、instagram.com は `check_date_updates.py` が
除外するので `shared_source_page_multi_date` の対象外。**
...
「`HTTPError`(4xx/5xx) は相手の返事なので引き直さない」と書いた。
**5xx をまとめて『返事』に入れたのが誤り。**
`503 Service Unavailable` は「今は無理、あとで来い」という意味で、
404/403 とは要求されている振る舞いが違う。

今日 `coverage_sweep_broken`(urgent) が
「2026-09-26: HTTP 503 / 2026-09-30: HTTP 503」で鳴った。
手元から同じURLを引くと200で返る。落ちているのは相手ではなくその回の応答で、
**09-12 に接続系で塞いだのと同じ形の雑音が、status 経由でもう一度入ってきた。**

直し: 引き直す status を `sitelib.RETRY_HTTP_STATUS`
(429 / 500 / 502 / 503 / 504) に置き、取得そのものを
`sitelib.fetch_text()` に寄せた。**巡回スクリプト2本が別々に持っていた
`fetch()` は、これで規則を1つだけ見る。**

検査は `sitelib.should_retry_status()` を coverage-sweep の自己テストから
8通りで当てる。**判定を純関数に切り出したのは、ネットワークを張らずに
検査できるようにするため。**`fetch_text` 全体を試すには `urlopen` を
差し替えるしかなく、CI から毎回走る自己テストでそれをやると、
差し替えが漏れた回に巡回そのものが壊れる。
引き直しの回数と順序は sandbox で `urlopen` を差し替えて実測した
(503×2→200 で3回・404 は1回で上げる・503 が続けば3回で上げる)。
...
### 項目名のラベルは、値の先頭に貼り付いたまま入る (2026-09-17)

スクレイプした行を項目に振り分けるとき、ラベルを剥がし忘れると値がこうなる。

> `admission: "入場料: 無料"`
> `description: "日時：2026年6月13日（土）12:00～21:00 会場：魚町みらい広場 日常に、野生を。…"`

詳細頁のスペック表は項目名を**自前で描く**ので、「入場料 / 入場料: 無料」と
同じ語が二度出る。値としては読めてしまうため、
`admission_is_free` も正しく無料と判定し、どの検査にも掛からなかった。

`enrich_events.py` は **venue についてだけ** 同じ剥がし処理を自前で持っていた
(`^(会場|開催場所|場所)\s*[：:]\s*` を素の正規表現で)。
つまり事故の型は既に知られていたのに、知っていたのが1フィールドの
書く側だけで、見張る側は誰も居なかった。**1か所で対処した事故は、
他のフィールドで同じ形のまま生き残る。**

規則は `sitelib.has_field_label_prefix` / `strip_field_label` に寄せ、
`audit.field_label_residue`(urgent) と `enrich_events.py` の
time / admission / access / venue が同じものを呼ぶ。

**先頭だけを見る。** 本文の途中の「主催: gunma.parryi_freaks。」は
読める日本語で、そこまで拾うと誤検知が止まらない(実測で3件→2件に絞った)。

### 住所は venue の先頭だけでなく、末尾の括弧にも入る (2026-09-17)

`venue_is_postal_address` は `〒` か `123-4567` で**始まる**値しか見ない。
施設名の後ろに括弧書きで住所を足した形は素通りしていた。

> `venue: "サンシャインシティ 展示ホールA（東京都豊島区東池袋3-1）"`
> `location: "サンシャインシティ"`

住所と会場名が venue と location で**逆に**入っていた。
この回は build 側が括弧を落としていたので表には出なかったが、
venue は location より優先されるので、落とす処理が無い経路では
Place.name とスペック表に住所がそのまま出る。

判定は「都道府県名 → 市/郡/区 → 数字」が1つの値の中で揃うこと
(`sitelib.venue_has_embedded_address`)。
「埼玉県営和光樹林公園」「東京都立小台橋高等学校」「やまぎん県民ホール」は
市区郡＋数字が続かないので掛からない。全451件で実測し、venue は1件だけ、
location(139件) と mapQuery(165件) は本来住所を持つ役割なので対象外にしてある。

**「〜で始まる」で書いた検査は、「〜を含む」の事故を必ず取りこぼす。**

### 主催公式サイトの「イベント一覧」を、誰も見ていなかった (2026-09-18)

`watch-sources.json` の `officialSiteCandidates` が持っているのは
**最後に掲載した回の個別ページのURL**である。

```
{"domain": "greensnap.jp",
 "url": "https://greensnap.jp/event/events/greensnap-marche-yokohama-2026-autumn/",
 "eventName": "GreenSnap Marche YOKOHAMA 2026 Autumn"}
```

個別ページには**その回しか書いていない。**同じ主催が自分のサイトに並べている
次の回は、何度この URL を開いても出てこない。

GreenSnap Marche 豊洲 2026 Autumn(10/31-11/1)と お台場 2026(11/28-29)が
これに当たった。どちらも主催公式 `greensnap.jp/event/` のイベント一覧に
並んでいるのに、**LEAFLA・NextMeet・PUKUBOOK のどれにも無く、
`coverage-gaps.json` も 0件**だった。
お台場は Google の `qdr:w2` でも1件も出ず、豊洲 Autumn の出店者募集投稿から
主催のトップへ上がって初めて2件とも見えた。

`watch-seeds.json`(2026-09-17)は「一度も掲載したことがない主催」の入口だが、
これは逆で、**何度も掲載している主催なのに、見ている窓が1回ぶんしかない。**
掲載実績があることが、かえって「見ているつもり」を作る。

対処は `manual-sweeps.json` の `organizer-official-index`(staleDays 14)。
`officialSiteCandidates` の domain ごとに、個別ページの1〜2階層上を開いて
`events.json` と並べる。**一覧を持っている主催は、一覧のほうを見る。**

### 出店者の常設取扱いの紹介文を、その回の品目と読み違えない (2026-09-18)

会場主催のイベント頁は、出店者を1者ずつ紹介するときに
**その店が普段扱っている品目**を書く。回の品目ではない。

> オザキフラワーパーク … 草花、観葉植物、庭木、**多肉植物**、野菜苗、園芸用品まで豊富に揃え

Green Market “AKEBONO” Selection(あけぼの山農業公園)の告知で、
`多肉植物` の語が出るのはここだけだった。回そのものの主題は
『季節の花苗や観葉植物、産地直送の生花販売』で、企画は花の競り体験と
ガーベラすくい、同時開催はパンフェス。

キーワードで当てる巡回は、この1語で範囲内に化ける。
**判定に使うのは、回の主題を言っている文だけ。**出店者紹介の段落は
「誰が来るか」であって「何を売る回か」ではない。

---

### 実機GASは `countSheetRows` を持つようになっていた (2026-09-18)

`new-inquiries.json` の `_note` と §3 の冒頭に
「現に動いている GAS は 2026-08-24 版で `countSheetRows` を持たない(2026-09-06 に判明)」
が残っていたが、**2026-09-07 の貼り直しで解消している。**
実機は `gasVersion` = `2026-09-08`、`scripts/google-apps-script.js` の
`CONFIG.SCRIPT_VERSION` と一致しており、`onFormSubmit` は
`data.sheetRows = countSheetRows()` を通る。
物証は 09-17 13:33 の `18f74b4`「フォーム受信」で、
GAS 自身が `sheetRows` を 6 → 7 に書き換えている。

結論の「どちらが書いてもよい」は変わらないが、**理由が変わる。**
以前は「GASが書けないからタスクが測る」、今は
**「GASはフォーム送信の瞬間しか書かないから、送信の無い日はタスクが測る」**。
`audit.py` の `sheet_measurement_stale` の `note` は既に後者で書けており、
散文の側だけが古いまま2週間近く残っていた。

**外部システムの状態を述べた文は、直した日に全箇所を grep する。**
`inquiryPii.deployment` は「外部側を確認した日で閉じる」と決めているのに、
09-07 に閉じたとき `countSheetRows` を語る散文は探していない。
`gas_script_undeployed` は版数の一致しか見ないので、
**版数が合った瞬間に、版数を根拠にした記述が全部古くなる。**
検査が緑になることは、その検査について書いた文が正しいことを意味しない。

### 同じ日の2回目の回は、1回目の成果を隠しうる (2026-09-18)

09-18 は 09:05 JST に1回動いて `1f7fcee`（プレミアム ロックガーデン フェスタ2026 の掲載・
受け渡し箱を空に）を押しており、10:20 JST に2回目が起動した。
2回目から見ると `items` は空・シート7行 = `processed` 7件で**新着ゼロ**だが、
それは**1回目が処理したから**であって、フォームが静かだったからではない。

- `record-run.py` は冪等なので台帳は壊れない。`reviewedHistory` に 09-18 は1つだけ。
- レポート名は `<taskId>_<日付>.md` で**日付までしか持たない**ので、
  2回目が素朴に「新着ゼロ」と書くと、その日の唯一の記録が
  1回目の掲載処理を落としたものになる。
- 1回目はレポートを残していない（§3「レポートが無くても、成果物を見れば動いたかが分かる」と同型）。

**新着ゼロと判定したら、その日の `git log` を必ず見る。**
同日に自分の処理済みコミットがあれば、レポートに「同日1回目が何をしたか」を書く。
`inquiries-processed.json` の `updated` が当日なら、その日は誰かが処理している。

### 機械に渡す値は、人が読んで正しくても壊れている (2026-09-18)

`mapQuery` は表示されない。`build-detail-pages.make_venue_map` が `quote()` して
`maps?q=<値>&output=embed` に入れる**引数**で、読者が見るのは地図そのものだけ。
`okibota-spring-2026-sunshine` の値は「サンシャインシティ+展示ホールA」で、
人が読めば正しい会場名だが、`quote()` は `+` を `%2B` に変えるので
Google Maps はリテラルの `+` として検索する。
テンプレート化した `650baa5`(2026-07) から入っており、**2か月以上ずれた地図を出していた。**

- 地図の枠は出る。何かしらの場所も出る。だから存在確認では捕まらない(§5)。
- 監査は `venue` / `location` / `mapQuery` を「会場名として成立しているか」で見ていたが、
  **符号化の残骸は日本語として成立している。**意味の検査は形式の事故を見ない。
- 逆に `venue` や `name` の `+` は「A+B」と読ませる正当な書き方がある。
  **同じ文字でも、機械の引数になる欄だけが壊れる。**検査は `mapQuery` に絞る。

`map_query_encoding_residue` で見る。

### 出典が一次情報でも、その行の「形」は保証されない (2026-09-18)

`gardens-umekita-2nd-anniv-2026-09` の `access` は
「JR大阪中央北出口駅より徒歩9分」。会場公式(`gardens.hc-kohnan.com`)を実際に取得すると
**出典側がその通り書いている。**「JR大阪駅 中央北出口」を字詰めで詰めた結果で、
`enrich_events` の access 抽出は1行を丸ごと取るのでそのまま入った。

出典の裏取りは「事実が合っているか」しか見ない。
**出典が正しいことは、値が項目の形として正しいことを保証しない。**
`access_pref_mismatch` は路線と県の食い違いを見るので、この形は素通しだった。

「出口 / 方角+口」の直後に「駅」が来る駅名は無い。
「川口駅」「山口駅」は方角でも出口でもなく、「浅草駅北口」は語順が逆なので掛からない。
`access_malformed_station` で見る。裏取りできない出口名は落として駅名だけ残す。

### 入口の条件に、欠けているその項目を置いてはいけない (2026-09-18)

`audit.instagram_embed_missing` は詳細頁にIG埋め込みが出ているかを**機能確認**する、
よくできた検査。だが最初の行が
`if not _pid and not _iurl: continue` で、**入口が `instagramPostId` の有無**になっている。

`events.json` 454件のうち 103件は `imageSource` に `instagram.com/p/<id>` を持つ
(アイキャッチをその告知投稿から取ったから)。うち **90件は `instagramPostId` を持たない。**
つまり告知投稿は判明しているのに、詳細頁はその節を丸ごと出さず、
検査は入口で全部を落としている。**「値が在る回が正しく出ているか」は、
「値が在るべき回に値があるか」を一切見ない。**

同じ事実が2か所に別の形で入る欄(`imageSource` と `instagramPostId`)があるときは、
**片方だけを入口にした検査は、もう片方しか持たない回をまとめて見逃す。**
欄を足したら、既にある検査の入口の条件を読み直すこと。

埋め込みを出すかは絵の重複の判断が要るので
`pending-judgments.json` の `event-monitor:ig-post-known-but-not-embedded` に積んだ。

**決着 (2026-09-20)。**写す。`scripts/sync-instagram-ids.py` が build-all の
生成より前で `imageSource` の投稿IDを `instagramPostId` に写す。85件に入り、
埋め込みのある詳細頁は 28 → 111 になった。

- **写すのは投稿IDだけ。**`instagramUrl` は `make_instagram_section` が
  投稿IDから組み立てる。両方に持つと必ず片方だけ直して食い違う。
- **`scripts/eyecatch-review.json` の回は写さない**(10件)。あれは
  「画像の出所がその回の告知投稿ではない」と目視で確かめた回
  (出店者募集・出店者紹介・会場図・店内写真)。画像は出典脚注付きの引用だが、
  **埋め込みは頁の上で「この回のInstagram投稿」と名乗る。**
  引用と名乗りでは要る確度が違う。
- 絵の重複は起きない。埋め込みはアイキャッチとは別の節で、
  出所を明示する導線としてむしろ筋が通る。
- 検査 `audit.instagram_id_unsynced` を足した。写せていない回が残っていたら
  スクリプトが黙って落ちている。**移行を手でやって終わりにすると、
  次にアイキャッチを取った回からまた片方だけになる。**
  一度の移行ではなく、毎回走る生成の一部にする。

### 作品展は「売るか」ではなく「主題か」で切る (2026-09-21 改定)

`outOfScopeGenres.artworkShows` は 2026-09-17 に「植物の即売が無い作品展は見送り」
として作った。`potteryAndVesselShows`(鉢・器の作家展)と同じ切り方を作品全般に広げたもの。
だが**読者が見に行く動機は即売だけではない。**アガベやサボテンそのものを描いた作品展は、
アガベ・ビザール好きにとって行く理由のある回で、主催も植物界隈の作り手である。

改めた規則は **作品の主題が対象植物そのものか。**
- 対象植物を描いた／象った作品が主役 → 掲載（植物の即売が無くてもよい）
- 鉢・器 → `potteryAndVesselShows` のまま見送り（主題が器）
- 植物一般・花のボタニカルアート、植物が背景でしかない作家展 → 見送り

初例は 植トゥザ・フューチャー2026 (GREEN PRICKLE 初個展・2026-10-16〜18 大阪)。
09-17 に「※植物の販売はございません。」を根拠に見送り、09-21 に掲載へ変更した。

**直接依頼は掲載の理由にしない。**この回は所属事務所(株)ヨンドから
Instagram DMで依頼が来たが、載せたのは規則を変えたからで、依頼が来たからではない。
依頼の有無で基準が動くなら、それは基準ではない。

付随して要るもの:

- 植物を売らない回は `plantSale: false` を入れる。`make_affiliate_block` が見て
  「株を持ち帰る前に揃えるもの」の枠を出さない。**株を持ち帰らない回にあの枠を出すと、
  書いてあることが嘘になる。**枠は遮光ネット・送風・薬剤を並べている。
- `coverage-sweep.py` の `OUT_OF_SCOPE` は '絵画' 等を持ったままにしてある。
...
**2026-09-14 の週次点検の記録。**
`task_run_gap` は 09-11 を3タスクとも落としている。`task-reports/` にも 09-11 付は1件も無い。
**全タスク同日**なので、上の規則どおり1行で閉じる(手元の実行環境の障害。09-09/09-10 の
shell 障害と同じ型で、§1.6 に手当て済み)。追わない。
`agave-navi-eyecatch` の 09-10〜09-13 は別で、`task_run_never_recorded` が
分けて出しているとおり SKILL.md 側の未修正。**repo 側では直せない**ので
...
## 「開催中」は本体の一覧に混ぜる (2026-09-18 決定)

専用の節は作らない。カードの「開催中 〜9/22」バッジで区別し、並び順は
`list_sort_key`(JS は `AEN_TIME.listSortKey`)が開催中を「今日」の位置に置く。
会期の長さで先頭に居座らないので、別枠を作った 2026-08-20 の理由は満たされる。

**件数をしきい値にした出し分けをやり直そうとしないこと。**
2026-09-08 に `status-auto.js` へ「開催中が1〜2件なら節を立てない
(`ONGOING_MIN = 3`)」を入れたが、**その判定は読み込み時に1回しか走らない。**
全国で6件あれば節が立ち、そのまま関東で絞ると2件になって4カラムの行が
ほぼ空になる。`top-filter.js` 側は見えている枚数で節の表示/非表示だけを
切り替えており、混ぜる判断はやり直さない。
2026-09-18 に「直したはずなのに先祖帰りしている」と指摘されたのはこの形で、
**コードは入っていたが絞り込み後には効いていなかった。**

一般化するとこう。**絞り込みで変わる数を、読み込み時に1回決める分岐の
条件にしてはいけない。**やるなら絞り込みのたびに再評価するか、
条件そのものを無くす。今回は無くした。

器を落とした先は4つ。`index.html` の `#ongoingHeading` / `#ongoingEventsGrid`、
`generate-landing-pages.events_section`、`sync-index-cards` の見出し書き換え、
`audit.section_note_missing` の対象。JS は `status-auto.arrangeList` が常に
`main` へ混ぜ、`top-filter.applyFilters` は開催中を扱わない。
