...
7. **台帳に `taskId` を足したら、同じ回にそのタスクの SKILL.md へも
   ...
   **「記録していない」と「動いていない」を先に切り分ける（2026-09-15）。**
   ...
   - **レポートも無い → タスクが動いていない。SKILL.md を直しても直らない。**
     Coworkデスクトップのスケジュールタスク一覧で、有効か・最後の実行が
     どうなったかを見る。

   2026-09-15、この検査を前者と決めつけて SKILL.md を2回直そうとし、
   目崎の時間を2往復ぶん使った。**実際は後者だった。**
   `agave-navi-eyecatch` はレポートも取得画像も 09-09 を最後にゼロで、
   その間にアイキャッチ残数は 41 → 54 に増えていた。
   **台帳が空なのは症状で、原因ではない。**成果の指標が悪化していないかを
   必ず併せて見る。指標が動いていれば記録の問題、止まっていればタスクの問題。
