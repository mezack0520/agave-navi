...
def make_affiliate_block(ev):
    ...
    植物の即売が無い回(plantSale=False)にも出さない。枠の見出しが
    「株を持ち帰る前に揃えるもの」で、鉢・用土・その月の管理用品を並べる。
    **株を持ち帰らない回に出すと、書いてあることが嘘になる。**
    2026-09-21 に artworkShows を『売るか』から『主題か』へ改め、
    植物を売らない作品展が載るようになったので必要になった
    (植トゥザ・フューチャー2026 が初例)。"""
    if sitelib.is_cancelled(ev):
        return ''
    if ev.get('plantSale') is False:
        return ''
