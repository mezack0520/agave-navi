...
OUT_OF_SCOPE = (
    ...
    '角鉢展', '器展', 'うつわ展',
    # 2026-09-21: artworkShows を『売るか』から『主題か』へ改めたので、
    # **対象植物そのものを描いた作品展は掲載対象になった。**
    # だが主題は語では決まらない。ここは候補を絞る粗い網のままにしてあり、
    # '絵画' 等で落ちた候補が掲載相当のことがある。網を緩めると
    # ボタニカルアート・花の絵画が大量に流れ込むので緩めない。
    # **この網に当たったからという理由で見送りを決めないこと。**
    # 落ちるのは候補の発見であって、判断ではない。
)
...
def sweep_nextmeet(days, sleep=0.7):
    ...
    # 先の窓に足りなくても当月+3か月は必ず見る。**月別の値打ちは
    # 45日の窓の外にある回**で、2026-09-06 に手で見つけた15件のうち5件は
    # 11/14 以降だった。日数で切ると、その5件をまた落とす。
    # 2026-09-13 に 3 → 4 か月へ広げた。9月中旬の窓(45日)は 10/28 までで、
    # 当月+2 でも 11月止まり。**その時点で主催は12月回を告知し始めている**
    # (路地裏ボタニカル 12/20 は 09-12、フラリエ 多肉X'mas 12/5-6 は 09-07、
