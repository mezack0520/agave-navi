...
# このファイルを直した push は daily.yml を発火させる(2026-09-17 に追加)。
# sitelib は生成物すべての単一情報源なので、再生成が走らないと
# 本番は翌朝まで旧版のままになる。
...
# 頁が「開催期間」「会期」などの肩書を付けて名乗った開始日。
# 肩書の無い裸の日付より**強い**。投稿日が肩書無しで題のそばに出る作りだと、
# 名前との近さで並べる extract_dates はそちらを先頭にしてしまう。
# 2026-09-23、プレミアム ロックガーデン フェスタ2026 の公式頁は題の直後に
# 「2026年9月9日 / 2026年9月16日」(投稿日と更新日、肩書なし)を出し、
# 本文の「開催期間：2026年9月19日（土）～ 9月27日（日）」より近かったため、
# CI が開始日を 09-19 から 09-09 へ書き換えていた。meta_dates は肩書を見るので
# この頁では効かない。肩書のある側を拾って、あればそちらだけを候補にする。
_PERIOD_LABEL_RE = re.compile(
    r'(?:開催期間|開催日時|開催日程|開催日|会期|日程|日時)[^0-9]{0,4}'
    r'(?:(\d{4})\s*年[^0-9]{0,2})?(\d{1,2})\s*[月/.]\s*(\d{1,2})')


def stated_period_dates(html):
    """肩書付きで名乗られた開始日の (月, 日) の集合。空なら肩書が無い頁。"""
    blob = page_text_blob(html)
    return {(int(m), int(d)) for _y, m, d in _PERIOD_LABEL_RE.findall(blob)
