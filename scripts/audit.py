...
def main():
    ...
    add('task_run_never_recorded', '台帳に登録したが一度も自分で書いていないタスク',
        task_never,
        note='**原因は2つある。先に切り分ける。**'
             '(A) タスクは動いているが記録していない … '
             'task-reports/<taskId>_<日付>.md が**ある**。'
             'そのタスクの SKILL.md に「起動直後に '
             'python3 scripts/record-run.py <taskId> を実行して push する」を'
             '入れる(プレイブック §1 の 5)。'
             '(B) タスクがそもそも動いていない … '
             'レポートも**無い**。SKILL.md をいくら直しても直らない。'
             'Coworkデスクトップのスケジュールタスク一覧で、'
             'そのタスクが有効か・最後の実行がどうなったかを見る。'
             '**2026-09-15、この検査を (A) と決めつけて SKILL.md を2回直そうとしたが、'
             '実際は (B) だった**(agave-navi-eyecatch はレポートも成果物も6日間ゼロで、'
             'アイキャッチ残数が 41→54 に増えていた)。'
             '成果の指標が悪化していないかを必ず併せて見る。'
             'なお SKILL.md は Claude 内部領域なので Cowork セッションからは書けない'
             '(2026-09-14 実測。親フォルダを接続しても protected location として拒否'
             'される)。目崎が手で貼るしかなく、フォルダ接続を頼む提案はしないこと',
        severity='info')
