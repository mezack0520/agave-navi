...
def main():
    ...
    # この分野で誰もが使う語。最長共通部分文字列の材料から先に抜く。
    # 残すと「ガーデン」だけで名称4字・会場4字の両方が当たり、
    # 無関係な回が一致と判定される。2026-09-23 に
    # 見送り「コーナン港北インター店…ガーデンズ」と掲載「第6回 ロビン
    # ビザールガーデン(会場ロビンガーデン)」が、共通部分「ガーデン」だけで
    # 衝突扱いになっていた。閾値を上げるのではなく、一般語を先に抜く。
    _GENERIC_RE = re.compile(
        '多肉植物|観葉植物|塊根植物|珍奇植物|ビザールプランツ|ビザール|プランツ'
        '|ガーデンズ|ガーデン|マルシェ|フェスティバル|フェスタ|フェス'
        '|即売会|展示会|販売会|イベント|サボテン|アガベ|植物'
        '|botanical|plants|plant|garden|marche|market|popup|pop-?up|fes(?:ta|tival)?'
        '|パーク|センター|ストア|ショップ|店')
    ...
    MANUAL_ONLY = {
        ...
        # CSSを機械的に書き換えるときの部品。単体では何もしない。
        # 正規表現で1行だけ消すと複数行グループの残りが宙に浮くので
        # (2026-09-08 に2箇所やった)、規則の単位で扱う道具を用意した
        'csslib.py',
    }
    ...
    add('task_run_never_recorded', '台帳に登録したが一度も自分で書いていないタスク',
        task_never,
        note='そのタスクのプロンプトに「起動直後に '
             'python3 scripts/record-run.py <taskId> を実行して push する」と'
             '「レポートを task-reports/<taskId>_<日付>.md に保存する」を入れる'
             '(プレイブック §1 の 5)。**編集するのは Cowork デスクトップの'
             'タスク画面の「手順」**(鉛筆アイコン)。'
             'Claude\\Scheduled\\<task>\\SKILL.md は Claude 内部領域で、'
             'Cowork セッションからは読み書きできない(2026-09-14 実測。'
             '親フォルダを接続しても protected location として拒否される)。'
             'フォルダ接続を頼む提案はしないこと。'
             '**動いている処理には触らない。**この検査が言っているのは'
             '「記録していない」だけで、「動いていない」ではない。'
             '2026-09-15〜16 に agave-navi-eyecatch をそう誤診して2日使った。'
             '実際は毎日走って画像を取って push までしていて、'
             '記録の2手順だけが抜けていた。'
             '**動いているかどうかは成果物のコミットで見る**'
             '(例: git log --since=... -- images/events/)。'
